    ...
    window.fetch(satellitesUrl).then(function(response) {
       response.json().then(function(data) {
          ...
          for (let i = 0; i < fetchedSatellites.length; i++) {
            let satellite = new Satellite(fetchedSatellites[i].name, fetchedSatellites[i].type, fetchedSatellites[i].fetchedSatellites, fetchedSatellites[i].orbitType, fetchedSatellites[i].operational);
